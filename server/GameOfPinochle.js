...
class GameOfPinochle {
    ...
    newGame() {
        ...
        for (let n = this.dealer + 1; n < this.dealer + 1 + this.NumberOfPlayers; n++) {
            let m = n % this.NumberOfPlayers;
            ...
            this.gamePoints[m] = 0;
            let myTrump = this.player[m].findBestTrump();
        ...
        // ** update statistics **
        for (let i = 0; i < this.CARDS_DEALT; i++) {
    ...
    cpuDiscardCards() {
        ...
        let discarded = this.player[this.starter].discardCards(this.talonSize, this.trump);
    ...
    showMelds() {
        ...
        for (let n = this.dealer + 1; n < this.dealer + 1 + this.NumberOfPlayers; n++) {
            ...
            if (m == this.starter) {
                ...
                let meldPoints = this.player[m].meldPoints;
                let trickPoints = this.player[m].trickPoints;
    ...
    playRound() {
        ...
        let playersRemaining = Math.pow(2, this.NumberOfPlayers + 1) - 2; // 2 + 4 + 8 + 16;
        let winningPlayer = this.starter;
        // *** each this.player plays a card ***
        for (let n = this.starter; n < this.starter + this.NumberOfPlayers; n++) {
            let current = n % this.NumberOfPlayers;
            playersRemaining &= ~(2 << current);
            let cardPlayed = this.player[current].playCard(this.cardsOnTable, this.trump, highestCard, playersRemaining);
        ...
        let pts = this.cardsOnTable.calculatePoints(this.round);
    ...
    playRounds() {
        ...
        for (let round = 1; round <= this.CARDS_DEALT; round++) {
            this.round = round;
            this.playRound();
        }
    }
